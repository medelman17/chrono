...
async function extractImageMetadata(buffer: Buffer, filename: string) {
  try {
    ...
    console.log(`[DEBUG] Metadata extracted from ${filename}:`);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (file.size > maxSize) {
      return NextResponse.json({ error: `File size exceeds limit of ${maxSize / 1024 / 1024}MB` }, { status: 400 });
    }
    ...
    const { error: uploadError } = await supabase.storage.from(STORAGE_BUCKETS.DOCUMENTS).upload(filePath, buffer, {
    ...
    const {
      data: { publicUrl },
    } = supabase.storage.from(STORAGE_BUCKETS.DOCUMENTS).getPublicUrl(filePath);
